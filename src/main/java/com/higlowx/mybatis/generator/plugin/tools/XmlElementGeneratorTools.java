...
public class XmlElementGeneratorTools {
    ...
    /**
     * 使用JDBC的getGeneratedKeys方法获取主键并赋值到keyProperty设置的领域模型属性中。所以只支持MYSQL和SQLServer
     *
     * @param element
     * @param introspectedTable
     * @param prefix
    ...
    public static void useGeneratedKeys(XmlElement element, IntrospectedTable introspectedTable, String prefix) {
        Optional<GeneratedKey> gk = introspectedTable.getGeneratedKey();
        if (gk.isPresent()) {
            IntrospectedColumn introspectedColumn = IntrospectedTableTools.safeGetColumn(introspectedTable, gk.get().getColumn());
